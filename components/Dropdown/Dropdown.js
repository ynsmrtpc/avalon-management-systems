...
export default function Dropdown({ buttonText, items, classes, labels }) {
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button
          className={`inline-flex w-full justify-center gap-x-1.5 rounded-md  px-3 py-2 text-sm font-semibold text-base-content ${classes}`}
        >
          {buttonText}
          <ChevronDownIcon
          ...
          <div className="py-1">
            {items.map((item, key) => (
              <Menu.Item key={key}>
                {({ active }) => (
                  <label
                    htmlFor={fn_make_label(labels[key])}
