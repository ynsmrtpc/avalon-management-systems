...
export default function BreadCrumb({ path }) {
  ...
  return (
    <nav
      className="flex px-5 py-3 text-base-content rounded-lg bg-base-200"
    ...
    >
      <ol className="inline-flex items-center space-x-1 md:space-x-3">
        <li className="inline-flex items-center">
          <Link
            href="/"
            className="inline-flex items-center text-sm font-medium text-base-content/40"
          >
            <svg
        ...
        {breadcrumbs.map((breadcrumb, index) => (
          <li key={index}>
            <div className="flex items-center">
              ...
              {breadcrumb.isLast ? (
                <span className="ml-1 text-sm font-medium md:ml-2 text-base-content/60">
                  {fn_make_label(breadcrumb.name)}
                </span>
              ) : (
                <Link
                  href={`/${breadcrumb.name}`}
                  className="ml-1 text-sm font-medium md:ml-2 text-base-content/40"
                >
                  {fn_make_label(breadcrumb.name)}
                </Link>
              )}
