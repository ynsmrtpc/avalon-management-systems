...
export default function BlogsModal({ content }) {
  ...
  const handleModalSubmit = () => {
    ...
    formData.append(
      "process",
      modalData.id !== undefined ? "update" : "insert"
    );
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5">
            ...
            inputValue={
              <>
                ...
                {authors.map((author) => (
                  <>
                    <option
                      selected={modalData.user_id === author.id}
                      value={author.id}
                    >
                      {author.name_surname}
                    </option>
        ...
        <div className="col-span-2">
          <CustomInput
            className={
              "block rounded py-1.5 w-full bg-[#f1f1f1f1] dark:bg-[#394051] px-3 focus:bg-white dark:focus:bg-card_bg_dark transition-[background-color] outline-[#4b5563]"
            }
            rows="7"
            onInputChange={(e) =>
              setModalData((prevState) => ({
