...
export default function About() {
  ...
  const fetchData = async () => {
    try {
      ...
      const updatedResult = {
        socialData: socialData.data[0],
        fetchAbout: fetchAbout.data[0],
      };

      setLoading(false);

      // Sosyal medya isimlerini almak için "data" objesini dolaşıyoruz
      const names = Object.keys(updatedResult.socialData).filter(
  ...
  const addSocialMedia = async () => {
    const foundSocialMedia = socialMediaNames.find((social) =>
      socialMedia.includes(social)
    );
    if (foundSocialMedia) {
      setLoadingSocial(true);
      const formDataSocial = new FormData();
      formDataSocial.append("action", "addSocial");
      formDataSocial.append(
        "data",
        JSON.stringify({
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="grid-cols-12 md:grid-cols-6">
              <Card cardTitle="Sosyal Medya">
                ...
                />
                <div className="grid grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 my-4 gap-5 mrd:gap-5 text-center mx-auto">
              ...
              <Card
                cardTitle={
                  <div className="flex justify-between">
                    ...
                    <button className="text-xs p-2.5 rounded text-white dark:bg-green-800 dark:hover:bg-green-700 bg-green-500 hover:bg-green-400">
                      Yeni Ekle
                    </button>
                  </div>
                }
